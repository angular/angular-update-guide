...
export const RECOMMENDATIONS: Step[] = [
  ...
  { possibleIn: 600, necessaryAsOf: 600, level: 1, step: 'node 8', action: 'Make sure you are using Node 8 or later', },
  { possibleIn: 600, necessaryAsOf: 600, level: 1, step: 'Update to CLI v6', action: 'Update your Angular CLI globally and locally, and migrate the configuration to the [new angular.json format](https://github.com/angular/angular-cli/wiki/angular.json)<br/><br/>`${packageManagerGlobalInstall} @angular/cli@next`<br/>`${packageManagerInstall} @angular/cli@next`<br/>`ng update @angular/cli --from=1 --migrate-only`<br/>', },
  { possibleIn: 600, necessaryAsOf: 600, level: 2, step: 'cli v6 scripts', action: 'Update any `scripts` you may have in your `package.json` to use the latest Angular CLI commands.', },
  { possibleIn: 600, necessaryAsOf: 600, level: 1, step: 'Update to Angular v6', action: 'Update all of your Angular framework packages to v6, and the correct version of RxJS and TypeScript.<br/><br/>`ng update @angular/core --next --force`<br/><br/>After the update, TypeScript and RxJS will flow types better, which may expose existing errors in your applications typings', },
  ...
  { possibleIn: 600, necessaryAsOf: 600, level: 1, step: 'Update Dependencies for v6', action: 'Update Angular Material to the latest version.<br/><br/>`ng update @angular/material --next`<br/><br/>This will also automatically migrate deprecated APIs.', },
  { possibleIn: 600, necessaryAsOf: 600, level: 1, step: 'Update Other Dependencies for v6', action: 'Use `ng update` or normal package manager tools to identify and updated other dependencies.', },
  { possibleIn: 600, necessaryAsOf: 600, level: 2, step: 'strictPropertyInitializer', action: 'If you have TypeScript configured to be strict, update your `tsconfig.json` to disable `strictpropertyinitialization` or move property initialization from `ngOnInit` to your constructor'},
  { possibleIn: 600, necessaryAsOf: 800, level: 1, step: 'update to RxJS 6', action: 'Remove deprecated RxJS 6 features using [rxjs-tslint auto update rules](https://github.com/ReactiveX/rxjs-tslint)'},
