...
export const RECOMMENDATIONS: Step[] = [
  { possibleIn: 200, necessaryAsOf: 400, level: ApplicationComplexity.Basic, step: 'Extends OnInit', action: 'Ensure you don\'t use `extends OnInit`, or use `extends` with any lifecycle event. Instead use `implements <lifecycle event>.`', },
  { possibleIn: 200, necessaryAsOf: 400, level: ApplicationComplexity.Advanced, step: 'Deep Imports', action: 'Stop using deep imports, these symbols are now marked with ɵ and are not part of our public API.', },
  { possibleIn: 200, necessaryAsOf: 400, level: ApplicationComplexity.Advanced, step: 'invokeElementMethod', action: 'Stop using `Renderer.invokeElementMethod` as this method has been removed. There is not currently a replacement.', },
  { possibleIn: 400, necessaryAsOf: 400, level: ApplicationComplexity.Basic, step: 'Non Animations Module', action: 'If you use animations in your application, you should import `BrowserAnimationsModule` from `@angular/platform-browser/animations` in your App `NgModule`.', },
  { possibleIn: 400, necessaryAsOf: 400, level: ApplicationComplexity.Medium, step: 'Native Form Validation', action: 'Angular began adding a `novalidate` attribute to form elements when you include `FormsModule`. To re-enable native forms behaviors, use `ngNoForm` or add `ngNativeValidate`.', },
  { possibleIn: 400, necessaryAsOf: 400, level: ApplicationComplexity.Advanced, step: 'RootRenderer', action: 'Replace `RootRenderer` with `RendererFactoryV2` instead.', },
  { possibleIn: 400, necessaryAsOf: 400, level: ApplicationComplexity.Advanced, ngUpgrade: true, step: 'downgradeInjectable', action: 'The return value of `upgrade/static/downgradeInjectable` has changed.', },
  { possibleIn: 400, necessaryAsOf: 400, level: ApplicationComplexity.Advanced, step: 'Animations Tests', action: 'If you use Animations and tests, add `mods[1].NoopAnimationsModule` to your `TestBed.initTestEnvironment` call.', },
  { possibleIn: 200, necessaryAsOf: 500, level: ApplicationComplexity.Advanced, step: 'DefaultIterableDiffer', action: 'Stop using `DefaultIterableDiffer`, `KeyValueDiffers#factories`, or `IterableDiffers#factories`', },
  { possibleIn: 400, necessaryAsOf: 500, level: ApplicationComplexity.Basic, step: 'Template Tag', action: 'Rename your `template` tags to `ng-template`', },
  { possibleIn: 400, necessaryAsOf: 500, level: ApplicationComplexity.Medium, step: 'OpaqueToken', action: 'Replace any `OpaqueToken` with `InjectionToken`.', },
  { possibleIn: 400, necessaryAsOf: 500, level: ApplicationComplexity.Advanced, step: 'DifferFactory', action: 'If you call `DifferFactory.create(...)` remove the `ChangeDetectorRef` argument.', },
  { possibleIn: 400, necessaryAsOf: 500, level: ApplicationComplexity.Advanced, step: 'ErrorHandler Parameter', action: 'Stop passing any arguments to the constructor for ErrorHandler', },
  { possibleIn: 400, necessaryAsOf: 500, level: ApplicationComplexity.Advanced, step: 'ngProbeToken', action: 'If you use ngProbeToken, make sure you import it from @angular/core instead of @angular/platform-browser', },
  { possibleIn: 400, necessaryAsOf: 500, level: ApplicationComplexity.Advanced, step: 'TrackByFn', action: 'If you use TrackByFn, instead use TrackByFunction', },
  { possibleIn: 500, necessaryAsOf: 500, level: ApplicationComplexity.Basic, step: 'i18n Pipe Change', action: 'If you rely on the date, currency, decimal, or percent pipes, in 5 you will see minor changes to the format. For applications using locales other than en-us you will need to import it and optionally `locale_extended_fr` from `@angular/common/i18n_data/locale_fr` and registerLocaleData(local).', },
  { possibleIn: 500, necessaryAsOf: 500, level: ApplicationComplexity.Advanced, step: 'gendir', action: 'Do not rely on `gendir`, instead look at using `skipTemplateCodeGen`. <a href=https://github.com/angular/angular/issues/19339#issuecomment-332607471" target="_blank">Read More</a>', },
  { possibleIn: 220, necessaryAsOf: 600, level: ApplicationComplexity.Basic, ngUpgrade: true, step: 'Dynamic ngUpgrade', action: 'Replace `downgradeComponent`, `downgradeInjectable`, `UpgradeComponent`, and `UpgradeModule` imported from `@angular/upgrade`. Instead use the new versions in `@angular/upgrade/static`', },
  { possibleIn: 400, necessaryAsOf: 600, level: ApplicationComplexity.Medium, step: 'Animations in Core', action: 'If you import any animations services or tools from @angular/core, you should import them from @angular/animations', },
  { possibleIn: 400, necessaryAsOf: 600, level: ApplicationComplexity.Advanced, step: 'ngOutletContext', action: 'Replace `ngOutletContext` with `ngTemplateOutletContext`.', },
  { possibleIn: 400, necessaryAsOf: 600, level: ApplicationComplexity.Advanced, step: 'collectionChangeRecord', action: 'Replace `CollectionChangeRecord` with `IterableChangeRecord`', },
  { possibleIn: 400, necessaryAsOf: 900, level: ApplicationComplexity.Advanced, step: 'Renderer', action: 'Anywhere you use Renderer, now use Renderer2', },
  { possibleIn: 400, necessaryAsOf: 600, level: ApplicationComplexity.Advanced, step: 'Router Query Params', action: 'If you use preserveQueryParams, instead use queryParamsHandling', },
  { possibleIn: 430, necessaryAsOf: 800, level: ApplicationComplexity.Basic, step: 'Http', action: 'If you use the legacy `HttpModule` and the `Http` service, switch to `HttpClientModule` and the `HttpClient` service. HttpClient simplifies the default ergonomics (you don\'t need to map to JSON anymore) and now supports typed return values and interceptors. Read more on [angular.io](https://angular.io/guide/http).', },
  { possibleIn: 430, necessaryAsOf: 600, level: ApplicationComplexity.Advanced, step: 'DOCUMENT in @angular/platform-browser', action: 'If you use DOCUMENT from @angular/platform-browser, you should start to import this from @angular/common', },
  { possibleIn: 500, necessaryAsOf: 600, level: ApplicationComplexity.Advanced, step: 'ReflectiveInjector', action: 'Anywhere you use ReflectiveInjector, now use StaticInjector', },
  { possibleIn: 500, necessaryAsOf: 550, level: ApplicationComplexity.Medium, step: 'Whitespace', action: 'Choose a value of `off` for `preserveWhitespaces` in your `tsconfig.json` under the `angularCompilerOptions` key to gain the benefits of this setting, which was set to `off` by default in v6.', },
  { possibleIn: 600, necessaryAsOf: 600, level: ApplicationComplexity.Basic, step: 'node 8', action: 'Make sure you are using <a href="http://www.hostingadvice.com/how-to/update-node-js-latest-version/" target="_blank">Node 8 or later</a>', },
  { possibleIn: 600, necessaryAsOf: 600, level: ApplicationComplexity.Basic, windows: false, step: 'Update to CLI v6', action: 'Update your Angular CLI, and migrate the configuration to the <a href="https://github.com/angular/angular-cli/wiki/angular-workspace" target="_blank">new angular.json format</a> by running the following:<br/><br/>`NG_DISABLE_VERSION_CHECK=1 npx @angular/cli@6 update @angular/cli@6`<br/>', },
  ...
  { possibleIn: 600, necessaryAsOf: 600, level: ApplicationComplexity.Medium, step: 'cli v6 scripts', action: 'Update any `scripts` you may have in your `package.json` to use the latest Angular CLI commands. All CLI commands now use two dashes for flags (eg `ng build --prod --source-map`) to be POSIX compliant.', },
  ...
  { possibleIn: 600, necessaryAsOf: 700, level: ApplicationComplexity.Advanced, step: 'ngModel on form control', action: 'Support for using the ngModel input property and ngModelChange event with reactive form directives has been deprecated in v6 and removed in v7.', },
  { possibleIn: 600, necessaryAsOf: 600, level: ApplicationComplexity.Medium, step: 'ngModelChange order', action: 'ngModelChange is now emitted after the value/validity is updated on its control instead of before to better match expectations. If you rely on the order of these events, you will need to begin tracking the old value in your component.', },
  ...
  { possibleIn: 700, necessaryAsOf: 700, level: ApplicationComplexity.Medium, material: true, step: 'v7 material changes', action: 'If you use screenshot tests, you\'ll need to regenerate your screenshot golden files as many minor visual tweaks have landed.'},
  { possibleIn: 700, necessaryAsOf: 800, level: ApplicationComplexity.Advanced, material: true, step: 'v7 material deprecations', action: 'Stop using `matRippleSpeedFactor` and `baseSpeedFactor` for ripples, using Animation config instead.'},
  { possibleIn: 800, necessaryAsOf: 800, level: ApplicationComplexity.Basic, windows: false, step: 'v8 update', action: 'Update to version 8 of the core framework and CLI by running `NG_DISABLE_VERSION_CHECK=1 npx @angular/cli@8 update @angular/cli@8 @angular/core@8` in your terminal and review and commit the changes.'},
  { possibleIn: 800, necessaryAsOf: 800, level: ApplicationComplexity.Basic, windows: true, step: 'v8 update', action: 'Update to version 8 of the core framework and CLI by running `cmd /C "set "NG_DISABLE_VERSION_CHECK=1" && npx @angular/cli@8 update @angular/cli@8 @angular/core@8"` in your terminal and review and commit the changes.'},
  ...
  { possibleIn: 800, necessaryAsOf: 800, level: ApplicationComplexity.Basic, step: 'TypeScript 3.4', action: 'Angular now uses TypeScript 3.4, [read more about errors that might arise from improved type checking](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-3-4.html).', },
  { possibleIn: 800, necessaryAsOf: 800, level: ApplicationComplexity.Basic, step: 'node 10', action: 'Make sure you are using <a href="http://www.hostingadvice.com/how-to/update-node-js-latest-version/" target="_blank">Node 10 or later</a>.', },
  ...
  { possibleIn: 800, necessaryAsOf: 800, level: ApplicationComplexity.Basic, windows: false, material: true, step: 'v8 material update', action: 'Update Angular Material to version 8 by running `NG_DISABLE_VERSION_CHECK=1 npx @angular/cli@8 update @angular/material@8` in your terminal.'},
  { possibleIn: 800, necessaryAsOf: 800, level: ApplicationComplexity.Basic, windows: true, material: true, step: 'v8 material update', action: 'Update Angular Material to version 8 by running `cmd /C "set "NG_DISABLE_VERSION_CHECK=1" && npx @angular/cli@8 update @angular/material@8"` in your terminal.'},
  { possibleIn: 800, necessaryAsOf: 900, level: ApplicationComplexity.Basic, material: true, step: 'deep imports', action: 'Instead of importing from `@angular/material`, you should import deeply from the specific component. E.g. `@angular/material/button`. `ng update` will do this automatically for you.'},
  ...
  { possibleIn: 800, necessaryAsOf: 800, level: ApplicationComplexity.Advanced, step: 'node-sass', action: 'We have switched from the native Sass compiler to the JavaScript compiler. To switch back to the native version, install it as a devDependency: `npm install node-sass --save-dev`.' },
  { possibleIn: 800, necessaryAsOf: 800, level: ApplicationComplexity.Advanced, step: 'schematics async', action: 'If you are building your own Schematics, they have previously been *potentially* asynchronous. As of 8.0, all schematics will be asynchronous.' },
  { possibleIn: 900, necessaryAsOf: 900, level: ApplicationComplexity.Basic, step: 'node 10.13', action: 'Make sure you are using <a href="http://www.hostingadvice.com/how-to/update-node-js-latest-version/" target="_blank">Node 10.13 or later</a>.' },
  ...
  { possibleIn: 900, necessaryAsOf: 900, level: ApplicationComplexity.Medium, step: 'create commits', action: 'You can optionally pass the `--create-commits` (or `-C`) flag to [ng update](https://angular.io/cli/update) commands to create a git commit per individual migration.' },
  { possibleIn: 900, necessaryAsOf: 900, level: ApplicationComplexity.Basic, windows: false, step: 'ng update v9', action: 'Run `NG_DISABLE_VERSION_CHECK=1 npx @angular/cli@9 update @angular/core@9 @angular/cli@9` which should bring you to version 9 of Angular.' },
  { possibleIn: 900, necessaryAsOf: 900, level: ApplicationComplexity.Basic, windows: true, step: 'ng update v9', action: 'Run `cmd /C "set "NG_DISABLE_VERSION_CHECK=1" && npx @angular/cli@9 update @angular/cli@9 @angular/core@9"` which should bring you to version 9 of Angular.' },
  ...
  { possibleIn: 900, necessaryAsOf: 900, level: ApplicationComplexity.Basic, windows: false, material: true, step: 'update @angular/material', action: 'Run `NG_DISABLE_VERSION_CHECK=1 npx @angular/cli@9 update @angular/material@9`.' },
  { possibleIn: 900, necessaryAsOf: 900, level: ApplicationComplexity.Basic, windows: true, material: true, step: 'update @angular/material', action: 'Run `cmd /C "set "NG_DISABLE_VERSION_CHECK=1" && npx @angular/cli@9 update @angular/material@9"`.' },
  ...
  { possibleIn: 800, necessaryAsOf: 900, level: ApplicationComplexity.Medium, step: 'es5browser', action: 'Remove any `es5BrowserSupport` flags in your `angular.json` and set your `target` to `es2015` in your `tsconfig.json`. Angular now uses your browserslist to determine if an ES5 build is needed. `ng update` will migrate you automatically.' },
  { possibleIn: 900, necessaryAsOf: 900, level: ApplicationComplexity.Medium, step: 'ngForm selector', action: 'If you use `ngForm` element selector to create Angular Forms, you should instead use `ng-form`.' },
  ...
  { possibleIn: 900, necessaryAsOf: 1000, level: ApplicationComplexity.Medium, step: 'debug', action: 'With Angular 9 Ivy is now the default rendering engine, for any compatibility problems that might arise, read the [Ivy compatibility guide](https://v9.angular.io/guide/ivy-compatibility).' },
  { possibleIn: 900, necessaryAsOf: 900, level: ApplicationComplexity.Advanced, step: 'express-universal-server', action: 'If you use Angular Universal with  `@nguniversal/express-engine` or `@nguniversal/hapi-engine`, several backup files will be created. One of them for `server.ts`. If this file defers from the default one, you may need to copy some changes from the `server.ts.bak` to `server.ts` manually.' },
  ...
  { possibleIn: 900, necessaryAsOf: 1000, level: ApplicationComplexity.Medium, step: 'entryComponents', action: 'In your application projects, you can remove `entryComponents` NgModules and any uses of `ANALYZE_FOR_ENTRY_COMPONENTS`. They are no longer required with the Ivy compiler and runtime. You may need to keep these if building a library that will be consumed by a View Engine application.' },
  { possibleIn: 900, necessaryAsOf: 1000, level: ApplicationComplexity.Medium, step: 'testbed-get', action: 'If you use `TestBed.get`, you should instead use `TestBed.inject`. This new method has the same behavior, but is type safe.' },
  { possibleIn: 900, necessaryAsOf: 1000, level: ApplicationComplexity.Medium, step: '$localize', action: 'If you use [Angular\'s i18n support](http://angular.io/guide/i18n), you will need to begin using `@angular/localize`. Learn more about the [$localize Global Import Migration](https://v9.angular.io/guide/migration-localize).' },

  { possibleIn: 1000, necessaryAsOf: 1000, level: ApplicationComplexity.Basic, step: 'v10 NodeJS 12', action: 'Make sure you are using <a href="https://nodejs.org/dist/latest-v12.x/" target="_blank">Node 12 or later</a>.' },
  { possibleIn: 1000, necessaryAsOf: 1000, level: ApplicationComplexity.Basic, step: 'ng update v10', action: 'Run `npx @angular/cli@10 update @angular/core@10 @angular/cli@10` which should bring you to version 10 of Angular.' },
  { possibleIn: 1000, necessaryAsOf: 1000, level: ApplicationComplexity.Basic, material: true, step: 'update @angular/material', action: 'Run `npx @angular/cli@10 update @angular/material@10`.' },
  { possibleIn: 1000, necessaryAsOf: 1000, level: ApplicationComplexity.Basic, step: 'browserlist', action: 'New projects use the filename `.browserslistrc` instead of `browserslist`. `ng update` will migrate you automatically.' },
  { possibleIn: 1000, necessaryAsOf: 1000, level: ApplicationComplexity.Medium, step: 'v10-versions', action: 'Angular now requires `tslint` v6, `tslib` v2, and [TypeScript 3.9](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-3-9.html). `ng update` will migrate you automatically.' },
  { possibleIn: 1000, necessaryAsOf: 1000, level: ApplicationComplexity.Advanced, step: 'styleext', action: 'Stop using `styleext` or `spec` in your Angular schematics. `ng update` will migrate you automatically.' },
  { possibleIn: 900, necessaryAsOf: 1000, level: ApplicationComplexity.Medium, step: 'classes-without-decorators', action: 'In version 10, classes that use Angular features and do not have an Angular decorator are no longer supported.  [Read more](https://v10.angular.io/guide/migration-undecorated-classes).  `ng update` will migrate you automatically.' },
  ...
  { possibleIn: 1000, necessaryAsOf: 1000, level: ApplicationComplexity.Medium, step: 'forms-number-input', action: 'If you use Angular forms, inputs of type `number` no longer listen to [change events](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/change_event) (this events are not necessarily fired for each alteration the value), instead listen for an [input events](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/input_event). ' },
  { possibleIn: 1000, necessaryAsOf: 1000, level: ApplicationComplexity.Medium, step: 'forms-length-input', action: 'For Angular forms validation, the `minLength` and `maxLength` validators now verify that the form control\'s value has a numeric length property, and only validate for length if that\'s the case.' },
  { possibleIn: 1000, necessaryAsOf: 1000, level: ApplicationComplexity.Medium, step: 'esm5-bundles', action: 'The [Angular Package Format](https://g.co/ng/apf) has been updated to remove `esm5` and `fesm5` formats. These are no longer distributed in our npm packages. If you don\'t use the CLI, you may need to downlevel Angular code to ES5 yourself.' },
  { possibleIn: 1000, necessaryAsOf: 1000, level: ApplicationComplexity.Medium, step: 'console-errors', action: 'Warnings about unknown elements are now logged as errors. This won\'t break your app, but it may trip up tools that expect nothing to be logged via `console.error`.' },
  { possibleIn: 1000, necessaryAsOf: 1000, level: ApplicationComplexity.Advanced, step: 'router-resolver-empty', action: 'Any resolver which returns `EMPTY` will cancel navigation. If you want to allow navigation to continue, you will need to update the resolvers to emit some value, (i.e. `defaultIfEmpty(...)`, `of(...)`, etc).' },
  { possibleIn: 1000, necessaryAsOf: 1000, level: ApplicationComplexity.Advanced, step: 'sw-vary-headers', action: 'If you use the Angular service worker and rely on resources with [Vary](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Vary) headers, these headers are now ignored to avoid unpredictable behavior across browsers. To avoid this, [configure](https://angular.io/guide/service-worker-config) your service worker to avoid caching these resources.' },
  ...
  { possibleIn: 1000, necessaryAsOf: 1000, level: ApplicationComplexity.Advanced, step: 'urlmatcher-null', action: 'If you use the `UrlMatcher`, the type now reflects that it could always return `null`.' },
  { possibleIn: 1000, necessaryAsOf: 1000, level: ApplicationComplexity.Basic, step: 'v10-more-details', action: 'For more details about deprecations, automated migrations, and changes visit the [guide angular.io](https://v10.angular.io/guide/updating-to-version-10)' },
  { possibleIn: 1020, necessaryAsOf: 1020, level: ApplicationComplexity.Medium, step: 'universal-baseurl', action: 'For Angular Universal users, if you use `useAbsoluteUrl` to setup `platform-server`, you now need to also specify `baseUrl`.' },

  { possibleIn: 1100, necessaryAsOf: 1100, level: ApplicationComplexity.Basic, step: 'v11 ng update', action: 'Run `ng update @angular/core@11 @angular/cli@11` which should bring you to version 11 of Angular.' },
  { possibleIn: 1100, necessaryAsOf: 1100, level: ApplicationComplexity.Basic, material: true, step: 'update @angular/material', action: 'Run `ng update @angular/material@11`.' },
  { possibleIn: 1100, necessaryAsOf: 1100, level: ApplicationComplexity.Basic, step: 'v11 versions', action: 'Angular now requires [TypeScript 4.0](https://devblogs.microsoft.com/typescript/announcing-typescript-4-0/). `ng update` will migrate you automatically.' },
  ...
  { possibleIn: 1100, necessaryAsOf: 1100, level: ApplicationComplexity.Advanced, step: 'viewencapsulation native removed', action: 'The component view encapsulation option `ViewEncapsulation.Native` has been removed. Use `ViewEncapsulation.ShadowDom` instead. `ng update` will migrate you automatically.' },
  { possibleIn: 1100, necessaryAsOf: 1100, level: ApplicationComplexity.Advanced, step: 'ICU expressions typechecked', action: 'If you use i18n, expressions within International Components for Unicode (ICUs) expressions are now type-checked again. This may cause compilation failures if errors are found in expressions that appear within an ICU. ' },
  ...
  { possibleIn: 1100, necessaryAsOf: 1100, level: ApplicationComplexity.Advanced, step: 'v11 slice pipe typing', action: 'The `slice` pipe now returns null for the undefined input value, which is consistent with the behavior of most pipes.' },
  ...
  { possibleIn: 1100, necessaryAsOf: 1100, level: ApplicationComplexity.Advanced, step: 'v11 number pipe typing', action: 'The number pipes (`decimal`, `percent`, `currency`, etc) now explicitly state which types are accepted.' },
  { possibleIn: 1100, necessaryAsOf: 1100, level: ApplicationComplexity.Advanced, step: 'v11 date pipe typing', action: 'The `date` pipe now explicitly states which types are accepted.' },
  ...
  { possibleIn: 1100, necessaryAsOf: 1100, level: ApplicationComplexity.Advanced, step: 'v11 async pipe typing', action: 'The `async` pipe no longer claims to return undefined for an input that was typed as undefined. Note that the code actually returned null on undefined inputs.' },
  { possibleIn: 1100, necessaryAsOf: 1100, level: ApplicationComplexity.Medium, step: 'v11 case pipe update', action: 'The `uppercase` and `lowercase` pipes no longer let falsy values through. They now map both `null` and `undefined` to `null` and raise an exception on invalid input (`0`, `false`, `NaN`). This matches other Angular pipes.' },
  ...
  { possibleIn: 1100, necessaryAsOf: 1100, level: ApplicationComplexity.Advanced, step: 'v11 CollectionChangeRecord', action: 'In change detection, `CollectionChangeRecord` has been removed, use `IterableChangeRecord` instead.' },
  ...
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Basic, step: 'v12 ng update', action: 'Run `ng update @angular/core@12 @angular/cli@12` which should bring you to version 12 of Angular.' },
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Basic, material: true, step: 'update @angular/material', action: 'Run `ng update @angular/material@12`.' },
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Basic, step: 'v12 versions', action: 'Angular now requires [TypeScript 4.2](https://devblogs.microsoft.com/typescript/announcing-typescript-4-2/). `ng update` will update you automatically.' },
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Basic, step: 'v12 browser support', action: 'IE11 support has been deprecated. Find details in the [RFC for IE11 removal](https://github.com/angular/angular/issues/41840).' },
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Basic, step: 'v12 minimum  Node.js version', action: 'You can no longer use Angular with Node.js version 10 or older' },
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Medium, step: 'v12 `XhrFactory` relocation', action: 'Change the import of `XhrFactory` from `@angular/common/http` to `@angular/common`.' },
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Medium, step: 'v12 i18n message ids', action: 'If you rely on legacy i18n message IDs use the `localize-migrate` tool to [move away from them](https://angular.io/guide/migration-legacy-message-id).' },
  ...
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Medium, step: 'v12 prod by default', action: 'You can run the optional migration for enabling production builds by default `ng update @angular/cli@12 --migrate-only production-by-default`.' },
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Advanced, step: 'v12 min and max form attributes', action: 'If you  use Angular forms, `min` and `max` attributes on `<input type="number">` will now trigger validation logic.' },
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Advanced, step: 'v12 `emitEvent` in `FormArray` and `FormGroup`', action: 'If your app has custom classes that extend `FormArray` or `FormGroup` classes and override the above-mentioned methods, you may need to update your implementation' },
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Advanced, step: 'v12 zone.js minimum version', action: 'Update zone.js to version 0.11.4. `ng update` will update this dependency automatically.' },
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Advanced, step: 'v12 `HttpParams` method params update', action: 'If you extend the `HttpParams` class you may have to update the signature of its method to reflect changes in the parameter types.' },
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Advanced, step: 'v12 `routerLinkActiveOptions`', action: '`routerLinkActiveOptions` property of `RouterLinkActive` now has a more specific type. You may need to update code accessing this property to align with the changes.' },
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Advanced, step: 'v12 `APP_INITIALIZER` callback types', action: 'The initializer callbacks now have more specific return types, which may require update of your code if you are getting an `APP_INITIALIZER` instance via `Injector.get` or `TestBed.inject`.' },
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Advanced, step: 'v12 fragment typings', action: 'The router fragments now could be `null`. Add `null` checks to avoid TypeScript failing with type errors.' },
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Advanced, step: 'v12 `ng.getDirectives`', action: 'Make sure you don\'t rely on `ng.getDirectives` throwing an error if it can\'t find a directive associated with a particular DOM node.' },
  { possibleIn: 1200, necessaryAsOf: 1200, level: ApplicationComplexity.Advanced, step: 'v12 `optimization.styles.inlineCritical`', action: 'Check out `optimization.styles.inlineCritical` option in your angular.json file. It now defaults to `true`. Remember that the whole `optimization` option can be set as boolean which will set all the suboptions to defaults.' },
  ...
  { possibleIn: 1300, necessaryAsOf: 1300, level: ApplicationComplexity.Medium, step: 'v13 service worker activated', action: 'The `activated` observable of `SwUpdate` is now deprecated. To check the activation status of a service worker use the `activatedUpdate` method instead.' },
  { possibleIn: 1300, necessaryAsOf: 1300, level: ApplicationComplexity.Medium, step: 'v13 service worker available', action: 'The `available` observable of `SwUpdate` is now deprecated. To get the same information use `versionUpdates` and filter only the `VersionReadyEvent` events.' },
  { possibleIn: 1300, necessaryAsOf: 1300, level: ApplicationComplexity.Medium, step: 'v13 renderModuleFactory', action: 'The `renderModuleFactory` from `@angular/platform-server` is no longer necessary with Ivy. Use `renderModule` instead.' },
  { possibleIn: 1300, necessaryAsOf: 1300, level: ApplicationComplexity.Advanced, step: 'v13 forms status', action: 'We narrowed the type of `AbstractControl.status` to `FormControlStatus` and `AbstractControl.status` to `Observable<FormControlStatus>`. `FormControlStatus` is the union of all possible status strings for form controls.' },
  { possibleIn: 1300, necessaryAsOf: 1300, level: ApplicationComplexity.Advanced, step: 'v13 router serializer', action: 'To align with the URI spec, now the URL serializer respects question marks in the query parameters. For example `/path?q=hello?&q2=2` will now be parsed to `{ q: `hello?`, q2: 2 }`' },
  ...
  { possibleIn: 1300, necessaryAsOf: 1300, level: ApplicationComplexity.Advanced, step: 'v13 removed symbols', action: 'The route package no longer exports `SpyNgModuleFactoryLoader` and `DeprecatedLoadChildren`. In case you use them, make sure you remove their corresponding import statements.' },
  ...
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Basic, material: true, step: 'update @angular/material', action: 'Run `ng update @angular/material@14`.' },
  ...
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Medium, material: true, step: 'v14 MatVertical and Horizontal Stepper', action: 'If you are using `MatVerticalStepper` or `MatHorizontalStepper` make sure you switch to `MatStepper`.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Medium, step: 'v14 JSONP', action: 'Remove headers from JSONP requests. JSONP does not supports headers and if specified the HTTP module will now throw an error rather than ignoring them.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Medium, step: 'v14 resolvers', action: 'Resolvers now will take the first emitted value by an observable and after that proceed to navigation to better align with other guards rather than taking the last emitted value.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, material: true, step: 'v14 deprecate protractor entry', action: 'The deprecated `angular/cdk/testing/protractor` entry point is now removed.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, material: true, step: 'v14 chipInput', action: 'Make sure you specify `chipInput` of `MatChipInputEvent` because it is now required.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, material: true, step: 'v14 mixinErrorState', action: 'You need to implement `stateChanges` class member in abstractions using `mixinErrorState` because the mixin no longer provides it.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, material: true, step: 'v14 CdkStepper orientation', action: 'Use `CdkStepper.orientation` instead of `CdkStepper._orientation`.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, material: true, step: 'v14 CdkStepper and MatStepper', action: 'If you are extending or using `CdkStepper` or `MatStepper` in the constructor you should no longer pass the `_document` parameter since it is now removed.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, material: true, step: 'v14 mat-list-item-avatar', action: 'Rename the `mat-list-item-avatar` CSS class to `mat-list-item-with-avatar`.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, material: true, step: 'v14 MatSelectionListChange.option', action: 'Use `MatSelectionListChange.options` rather than `MatSelectionListChange.option`.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, material: true, step: 'v14 getHarnessLoaderForContent', action: 'Use `getChildLoader(MatListItemSection.CONTENT)` rather than `getHarnessLoaderForContent`.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, material: true, step: 'v14 MatSelectionList', action: 'If you are using `MatSelectionList` make sure you pass `_focusMonitor` in its constructor because it is now required. Additionally, this class no longer has `tabIndex` property and a `tabIndex` constructor parameter.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, step: 'v14 initialNavigation', action: 'Update `initialNavigation: \'enabled\'` to `initialNavigation: \'enabledBlocking\'`.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, step: 'v14 Route.pathMatch', action: 'If you are defining routes with `pathMatch`, you may have to cast it to `Route` or `Routes` explicitly. `Route.pathMatch` is no longer compatible with `string` type.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, step: 'v14 stricter LoadChildrenCallback', action: 'The promise returned by `LoadChildrenCallback` now has a stricter type parameter `Type<any>|NgModuleFactory<any>` rather than `any`.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, step: 'v14 router scheduling', action: 'The router does no longer schedule redirect navigation within a `setTimeout`. Make sure your tests do not rely on this behavior.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, step: 'v14 LocationStrategy', action: 'Implementing the `LocationStrategy` interface now requires definition of `getState()`.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, step: 'v14 http queries', action: 'Sending `+` as part of a query no longer requires workarounds since `+` no longer sends a space.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, step: 'v14 AnimationDriver.getParentElement', action: 'Implementing `AnimationDriver` now requires the `getParentElement` method.' },
  { possibleIn: 1400, necessaryAsOf: 1400, level: ApplicationComplexity.Advanced, step: 'v14 invalid config', action: 'Invalid route configurations of lazy-loaded modules will now throw an error rather than being ignored.' },
  ...
  { possibleIn: 1700, necessaryAsOf: 1700, level: ApplicationComplexity.Basic, step: 'v17 zone.js support', action: 'Make sure that you are using a supported version of Zone.js before you upgrade your application. Angular v17 supports Zone.js version 0.14.x or later.' },
  { possibleIn: 1700, necessaryAsOf: 1700, level: ApplicationComplexity.Basic, step: 'v17 ng update', action: 'In the application\'s project directory, run `ng update @angular/core@17 @angular/cli@17` to update your application to Angular v17.' },
  ...
  },
  {
    ...
    action: "Import `StateKey` and `TransferState` from `@angular/core` instead of `@angular/platform-browser`."
  },
  {
    possibleIn: 1800,
    necessaryAsOf: 1800,
    ...
    action: "Use `includeRequestsWithAuthHeaders: true` in `withHttpTransferCache` to opt-out of caching for HTTP requests that require authorization."
  },
  {
    possibleIn: 1800,
    necessaryAsOf: 1800,
  ...
  },
  {
    ...
    action: "Remove calls to `Testability` methods `increasePendingRequestCount`, `decreasePendingRequestCount`, and `getPendingRequestCount`. This information is tracked by ZoneJS."
  },
  {
    possibleIn: 1800,
    necessaryAsOf: 1800,
    ...
    action: "Move any environment providers that should be available to routed components from the component that defines the `RouterOutlet` to the providers of `bootstrapApplication` or the `Route` config."
  },
  {
    possibleIn: 1800,
    necessaryAsOf: 1800,
  ...
  },
  {
    ...
    action: "Remove dependencies of `RESOURCE_CACHE_PROVIDER` since it's no longer part of the Angular runtime."
  },
  {
    possibleIn: 1800,
    necessaryAsOf: 1800,
    ...
    action: "In `@angular/platform-server` now `pathname` is always suffixed with `/` and the default ports for http: and https: respectively are 80 and 443."
  },
  {
    possibleIn: 1800,
    necessaryAsOf: 1800,
    ...
    action: "Provide an absolute `url` instead of using `useAbsoluteUrl` and `baseUrl` from `PlatformConfig`."
  },
  {
    possibleIn: 1800,
    necessaryAsOf: 1800,
    ...
    action: "Replace the usage of `platformDynamicServer` with `platformServer`. Also, add an `import @angular/compiler`."
  },
  {
    possibleIn: 1800,
    necessaryAsOf: 1800,
    ...
    action: "Remove all imports of `ServerTransferStateModule` from your application. It is no longer needed."
  },
  {
    possibleIn: 1800,
    necessaryAsOf: 1800,
    ...
    action: "`Route.redirectTo` can now include a function in addition to a string. Any code which reads `Route` objects directly and expects `redirectTo` to be a string may need to update to account for functions as well."
  },
  {
    possibleIn: 1800,
    necessaryAsOf: 1800,
    ...
    action: "`Route` guards and resolvers can now return a `RedirectCommand` object in addition to a `UrlTree` and `boolean`. Any code which reads `Route` objects directly and expects only `boolean` or `UrlTree` may need to update to account for `RedirectCommand` as well."
  },
  {
    possibleIn: 1800,
    necessaryAsOf: 1800,
  ...
  },
  {
    ...
    action: "Be aware that newly created views or views marked for check and reattached during change detection are now guaranteed to be refreshed in that same change detection cycle."
  },
  {
    possibleIn: 1800,
    necessaryAsOf: 1800,
    ...
    action: "After aligning the semantics of `ComponentFixture.whenStable` and `ApplicationRef.isStable`, your tests may wait longer when using `whenStable`.",
  },
  {
    possibleIn: 1800,
    necessaryAsOf: 1800,
