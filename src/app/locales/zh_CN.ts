...
const uiLabels: LocalizedLabels = {
  ...
  "There aren't currently any changes needed after moving between these versions.":
    '目前在这两个版本间变化后没有任何改变。',
  'for': '于',
  'Basic Apps': '基本（Basic） APP',
  'Medium Apps': '中等（Medium） APP',
  'Advanced Apps': '高级（Advanced） APP',
};
...
const steps: LocalizedSteps = {
  ...
  'i18n Pipe Change': {
    action: '如果您依赖日期（date），货币（currency），十进制（decimal）或百分比管道（percent pipes），则在 5 中您会看到格式的细微变化。除了 en-us 语言环境的应用，都需要选择从 `@angular/common/i18n_data/locale_fr` 和 registerLocaleData（local） 中导入 `locale_extended_fr`。'
  },
  'gendir': {
  ...
  'update to RxJS 6': {
    action: '移除 RxJS 5 不推荐使用的功能使用 <a href="https://github.com/ReactiveX/rxjs-tslint" target="_blank">rxjs-tslint</a> 自动更新规则<br/>在大多数应用中，这需要运行以下两个命令:<br/>`npx rxjs-tslint`<br/>`rxjs-5-to-6-migrate -p src/tsconfig.app.json`'
  },
  'remove rxjs-compat': {
