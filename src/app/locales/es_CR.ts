...
const steps: LocalizedSteps = {
  ...
  RootRenderer: {
    action:
      'Reemplace RootRenderer por `RendererFactoryV2`.'
  },
  downgradeInjectable: {
    action:
      'El valor de retorno de `upgrade/static/downgradeInjectable` cambió.'
  },
  'Animations Tests': {
    action:
  ...
  DefaultIterableDiffer: {
    action:
      'No utilice `DefaultIterableDiffer`, `KeyValueDiffers#factories`, o `IterableDiffers#factories.`'
  },
  'Template Tag': {
    action:
      'Renombre las etiquetas `template` a `ng-template`.'
  },

  'OpaqueToken': {
    action:
      'Reemplace `OpaqueToken` por `InjectionToken`.'
  },
  'DifferFactory': {
    action:
      'Si utiliza `DifferFactory.create(...)` remueva `ChangeDetectorRef` como argumento.'
  },
  'ErrorHandler Parameter': {
    action:
      'No pase ningún argumento al constructor de ErrorHandler'
  },
  'ngProbeToken': {
    action:
      'Si usa ngProbeToken. asegúrese de importarlo de `@angular/core` en vez de `@angular/platform-browser`'
  },
  'TrackByFn': {
    action:
      'Utilice TrackByFunction en vez de TrackByFn'
  },
  'i18n Pipe Change': {
    action:
  ...
  'ngOutletContext': {
    action:
      'Reemplace `ngOutletContext` por `ngTemplateOutletContext`.'
  },
  'collectionChangeRecord': {
    action:
      'Reemplace `CollectionChangeRecord` por `IterableChangeRecord`'
  },
  'Renderer': {
    action:
      'En todos los lugares que este utilizando Renderer, remplácelo por Renderer2'
  },
  'Router Query Params': {
    action:
      'En caso de utilizar preserveQueryParams, utilice queryParamsHandling en vez'
  },
  'Http': {
    action:
  ...
  'DOCUMENT in @angular/platform-browser': {
    action:
      'Si utiliza DOCUMENT de @angular/platform-browser, debería empezar a importarlo desde @angular/common'
  },
  'ReflectiveInjector': {
    action:
      'Reemplace ReflectiveInjector por StaticInjector'
  },
  'Whitespace': {
    action:
  ...
  'remove rxjs-compat': {
    action:
      'Una vez que todas las dependencias han sido actualizadas a RxJS 6, remueva `rxjs-compat`.'
  },
  ...
  'Node 10': {
    action:
      'Angular ahora soporta Node 10: https://nodejs.org/en/blog/release/v10.0.0/'
  },
  'v7 update': {
    action:
  ...
  'v7 material deprecations': {
    action:
      'No utilice `matRippleSpeedFactor` ni `baseSpeedFactor` para ripples, use la configuración de Animation'
  },
  'v8 update': {
    action:
  ...
  'v8 material update': {
    action:
      'Actualiza Angular Material a la versión 8 corriendo `ng update @angular/material@8` en la terminal.'
  },
  'deep imports': {
    action:
  ...
  'ng update v9': {
    action:
      'Corra `ng update @angular/core@9 @angular/cli@9` para actualizar a la versión 9 de Angular.'
  },
  'typescript 3.8': {
    action:
  ...
  'update @angular/material@9': {
    action:
      'Corra `ng update @angular/material@9`.'
  },
  ...
  'ngForm selector': {
    action:
      'Si utiliza el selector de `ngForm` para crear formularios en Angular, reemplácelo por `ng-form`.'
  },
  'typings compilation': {
    action:
  ...
  'urlmatcher-null': {
    action:
      'Si utilza `UrlMatcher`, ahora el tipo muestra que puede que siempre retorne `null`.'
  },
  'v10-more-details': {
    action:
